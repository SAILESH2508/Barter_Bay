...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($email) && !empty($password)) {
        ...
        $admin_stmt = $conn->prepare("SELECT id, email, password, name FROM admins WHERE email = :email");
        $admin_stmt->bindValue(':email', $email, SQLITE3_TEXT);
        $admin_result = $admin_stmt->execute();
        $admin_row = $admin_result->fetchArray(SQLITE3_ASSOC);

        if ($admin_row && password_verify($password, $admin_row['password'])) {
            // Admin login successful
            $_SESSION['admin'] = $admin_row['email'];
